...
Проект включает автоматически генерируемую Swagger документацию API. После запуска сервера главная страница будет доступна по адресу:

```
http://localhost:8080/
```

На главной странице представлена общая информация о сервисе и ссылки на документацию.
...
Это создаст файлы документации в директории `./docs/`.

### Структура API

API разделен на следующие группы:

- **auth** - Операции аутентификации и регистрации
- **secrets** - Операции с секретами (текст, логины/пароли, карты)
...
3. **Конфликты и синхронизация** — делаем optimistic update с передачей версии с клиента и 409 ошибкой, если на сервере хранится версия ≠ переданной с клиента. Это проще, чем делать очереди на запись, и позволит избежать блокировок, например, если одновременно передают большие файлы с одним лейблом.

5. **Хранение данных**. PostgreSQL — для хранения метаданных, данных пользователей, зашифрованных данных логин-пароль, текстов, данных карт и общей информации о файле-секрете. MinIO — для хранения зашифрованных файлов и других бинарных данных.

## Ограничения

1. Пользователь не сможет поменять пароль.
2. При удалении — удаляем из БД и MinIO.